...
Color Graphics::getPixel(Location &loc) const
{
	if (loc.X() < 0 || loc.Y() < 0 || loc.X() >= int(Graphics::ScreenWidth) || loc.Y() >= int(Graphics::ScreenHeight)) {
		return Colors::Black;
	}
	return pSysBuffer[Graphics::ScreenWidth * loc.Y() + loc.X()];
}
